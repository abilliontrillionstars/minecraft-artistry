...
--- EMOTES / VISUALS ---
------------------------
function pings.playAnim(anim) animations.truemoon[anim]:play() end
function pings.stopAnim(anim) animations.truemoon[anim]:stop() end
...
function handleWings()
    if host:isFlying() or player:isGliding() then
        ...
        models.truemoon.Body.WingL:setRot(0,-10+ math.sin(world.getTime()) * 15,0)
        models.truemoon.Body.WingR:setRot(0,10+ math.sin(world.getTime()) * -15,0)
        --oh also if the wings aren't open, open them
        if not EMPOWERED then pings.playAnim("wingsOut1") end
    else
        --wing breathing
        models.truemoon.Body.WingL:setRot(0,-10+ math.sin(world.getTime() / 25) * 4,0)
        models.truemoon.Body.WingR:setRot(0,10+ math.sin(world.getTime() / 25) * -4,0)
    end
...
function handleCharging()
    if chargeKey:isPressed() then
        ...
        if animations.truemoon.charge1loop:isPlaying() then
            pings.stopAnim("charge1loop")
            pings.playAnim("charge1shoot")
        elseif animations.truemoon.charge1start:isPlaying() then
            pings.stopAnim("charge1start")
            pings.playAnim("charge1shoot")
        elseif animations.truemoon.shieldstart:isPlaying() then
            pings.stopAnim("shieldstart")
            pings.playAnim("shieldshoot")
        elseif animations.truemoon.shieldloop:isPlaying() then
